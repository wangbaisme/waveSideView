...
import com.gdszzy.test03.wavesidebar.utils.FirstLetterUtil;
...
public class ContactModel {
    ...
    public static class ContactGroup {
        ...
        public ContactGroup(String index, ArrayList<ContactModel> list) {
            this.index = index;
            this.list = list;
        }
        public String getIndex() {
    ...
    public static List<ContactGroup> getContactGroup(List<ContactModel> list) {
        List<ContactGroup> contactGroups = new ArrayList<>();
        for (ContactModel contactModel : list) {
            boolean isContactModelExist = false;
            for (ContactGroup contactGroup : contactGroups) {
                if (contactGroup.index.equals(contactModel.index)) {
                    isContactModelExist = true;
                    contactGroup.list.add(contactModel);
                    break;
                }
            }
            if (!isContactModelExist) {
                ArrayList<ContactModel> data = new ArrayList<>();
                data.add(contactModel);
                contactGroups.add(new ContactGroup(contactModel.index, data));
            }
        }
